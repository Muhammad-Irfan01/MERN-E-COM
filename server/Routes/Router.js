...
Router.get('/getSingleProduct/:id', async(req, res) =>{
    try {
        ...
        console.log('product is' + getSingleProduct)
    ...
    try {
        ...
        if(!LoginCredentials){
           return res.status(400).json({message : 'User not found'})
            ...
            const checkPassword = await bcrypt.compare(password, LoginCredentials.password);
            if(!checkPassword){
               return res.status(400).json({message : 'Invalid Password'})
            }else{
                const accessToken = jwt.sign({id : LoginCredentials._id}, Access_Token_Key, {expiresIn : "15m"});
                const refreshToken = jwt.sign({id : LoginCredentials._id}, Refresh_Token_Key, {expiresIn : "7d"});
...
Router.post('/Refresh', async(res, req) =>{
    ...
    if(!refreshToken) return res.status(500).json({message : 'refreshToken required'});

    jwt.verify(refreshToken, Refresh_Token_Key, (err, user) =>{
        if(err) return res.status(500).json({error : 'invalid refresh token'});

        const accessToken = jwt.sign({id : user._id}, Access_Token_Key, {expiresIn : '15m'})
